...
// Reps per phase per type
function getReps(phase, type, baseReps) {
  const compound = [15, 12, 8, 5];
  const isolation = [20, 15, 12, 10];
  if (type === "compound") return compound[phase];
  // For isolation, scale relative to base but use table
  return isolation[phase];
}
...
export default function FitnessApp() {
  ...
  useEffect(() => {
    ...
    saveState({ weights, cycleStart, pausedWeeks, pauseActive, pauseStartDate });
  }, [weights, cycleStart, pausedWeeks, pauseActive, pauseStartDate, initialized]);
